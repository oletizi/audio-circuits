...
### Two known gaps carried forward rather than closed
...
**The residual, measured, and narrower than first stated.** The parameters' PRESENCE is pinned, indirectly: deleting `IS=4.352e-9` from `lib/sim/models/1N4148.spice` turns the suite RED, with `vactrolSubcircuit: model "1N4148" declares no plain-numeric "IS" ... Parsed keys: N, BV, RS, CJO, VJ, M, FC, TT`. The guard fires and every vactrol test goes down with it. **What is NOT pinned is the guard itself.** Replacing `requiredParameter`'s throw with `parsed.get(key) ?? 1` leaves the suite GREEN at 287 pass / 0 fail, because with the parameters present the fallback never fires. So a future edit that weakens the throw into a fallback is invisible today, and would only surface later, as a silently wrong offset, the first time a parameter went missing. The same holds for the two throws in `modelParameters` (no `.model` line; no parenthesised list), which no test feeds a malformed input. Closing it means either a seam that lets a test pass a hand-written `.model 1N4148 D(N=1.9)` string, or a registry-level assertion that the entry carries the three parameters the offset arithmetic reads.
