...
- **Every part and module test FILE must call `expectNoFailedComponents` at least once.** One call suffices per file: the helper scans the entire circuit-JSON element array, not just the components a given test names, so a single call covers every component in that fixture. An invalid footprint makes a component fail SILENTLY — absent from circuit JSON, one `source_failed_to_create_component_error` element, no exception thrown. A test can pass green while a component is missing. Measured: of the footprints this project uses, only `do214ab` was invalid, but the guard must be present so a future bad footprint cannot slip through. It matches that error type SPECIFICALLY — PCB autorouter errors are expected at this stage and must not trip it.
- **Every part and module test file must also call `expectNoFloatingPins`.** An unwired pin is a wiring bug, and in this project it is the single most likely one — the module has ~40 components and every pin is meant to be connected. If it fails, fix the wiring; reach for its allowlist only for a pin that is genuinely, deliberately unused.
...
// Every one of the four vactrol pins gets its OWN net, each energised
// through a real component. This matters for the isolation test below:
// if LED_K and LDR_2 both went straight to GND they would share a net
// externally, and an internal short between exactly those two pins —
// the classic symptom of a wrong pin-to-pad mapping — would be
// undetectable. Separate return nets also mirror the real circuit, where
// LED_K reaches the driver through a sense resistor and LDR_2 sits on
// VBIAS, neither of them on ground.
const fixture = (
  <board width="20mm" height="20mm">
...
test("LED and LDR sides are electrically isolated", async () => {
  ...
  // All FOUR cross-pairs must be asserted. Note expectNotConnected also
  // passes for a floating pin, so this is only meaningful because the
  // fixture energises every pin through a real component.
...
test("the isolation fixture energises all four pins on distinct nets", async () => {
  // Guards the test above: if a later edit collapsed two vactrol pins onto
  // a shared net, the isolation assertions covering that pair would start
